import {
  ...
  getQueryParams,
  buildRedirectUrl,
  persistPreAuthFlowHref,
  clearAuthStateEntries,
  restoreHref,
  getPreAuthFlowHref,
} from "./utils";
...
export const authenticate = async (
  {
    ...
    location,
    localStorage,
  }: Pick<Window, "history" | "location" | "localStorage">,
  oathClientID: string,
  ...
  if (oauthParams.state) {
    ...
    appState = getEmbeddedAppState(
      getPreAuthFlowHref(localStorage, oauthParams.state)
    );
  } else {
...
export const extractAccessToken = (authData?: IAuthData) : string => {
