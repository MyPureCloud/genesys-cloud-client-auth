...
} from "../../src/lib/authenticator";
import {
  EmbeddedAppState,
  getEmbeddedAppState,
  parseOauthParams as _parseOauthParams,
...
jest.mock("../../src/lib/authenticator");
jest.mock("../../src/lib/utils", () => {
  ...
  return {
    _esModule: true,
    ...originalModule,
    persistPreAuthFlowHref: jest.fn(),
    clearAuthStateEntries: jest.fn(),
    parseOauthParams: jest.fn(),
    restoreHref: jest.fn(),
    getPreAuthFlowHref: jest.fn(),
  };
});
jest.mock("../../src/lib/handlePopupWindowRedirect");
...
describe("authenticate", () => {
  ...
  it("should trigger the same-window login flow and handle when the promise resolves", async () => {
    ...
    expect(persistPreAuthFlowHref).toHaveBeenCalledWith(
      localStorage,
      location.href
    );
  ...
  it("should return auth failed state if an error is thrown during login (pop-up blocked)", async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith("Unable to authenticate", {
      reason: error,
    });
  });
});
